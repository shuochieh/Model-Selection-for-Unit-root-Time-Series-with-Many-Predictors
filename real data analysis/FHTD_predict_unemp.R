### Example:: U.S. Unemployment Forecasting ###
...
trans.output = function( vdata, method ){
  if( method == 1 ){ output = vdata[-c(1:2)] }
  if( method == 2 ){ output = diff( vdata, differences=1 )[-1] }
  if( method == 3 ){ output = diff( vdata, differences=2 ) }
  if( method == 4 ){ output = log( vdata )[-c(1:2)] }
  if( method == 5 ){ output = diff( log(vdata), differences=1 )[-1] }
  if( method == 6 ){ output = diff( log(vdata), differences=2 ) }
  if( method == 7 ){
    output = diff( diff( vdata, differences=1 )/vdata[-length(vdata)] , differences=1 )
  }
  return( output )
}

data.transform = function (data, variable, scale = FALSE) {
  # variable: IP, inflation, unemployment, 3mtbill, 5yTbond, 
  
  data  = data[-c(nrow(data)-1, nrow(data)), ] # omit last observation
  trans = data[1,]
  data  = data[170:nrow(data),] # omit observations before 12/1/1972
  
  if( variable=="IP" ){
    y = data[ -c(1:2), "INDPRO" ]
    X = data[,! colnames(data) %in% c("INDPRO")]
    trans.new = trans[ ! colnames(data) %in% c("INDPRO") ]
    
    trans.new = trans.new[ colSums(is.na(X)) == 0 ]
    X         = X[, colSums(is.na(X)) == 0 ]
    
    newdata   = matrix(NA, ncol = ncol(X), nrow = nrow(X) - 2)
    for (i in 1:ncol(X)) { 
      newdata[,i] = trans.output(vdata = X[,i], method = trans.new[i])
    }
    
    if (scale) {
      newdata = t(t(newdata) - colMeans(newdata))
      newdata = t(t(newdata) / sqrt(colSums(newdata^2)))
    }
    
    newdata = cbind(y, newdata)
  }
  if( variable=="inflation" ){
    y = ( diff( data[, "CPIAUCSL" ], differences=1 )/data[-length(data[, "CPIAUCSL" ]), "CPIAUCSL" ] *100 )[-c(1)]
    X = data[,! colnames(data) %in% c("CPIAUCSL")]
    trans.new = trans[ ! colnames(data) %in% c("CPIAUCSL") ]
  ...
  if( variable=="unemployment" ){
    y = data[ -c(1:2), "UNRATE" ]
    X = data[,! colnames(data) %in% c("UNRATE")]
    trans.new = trans[ ! colnames(data) %in% c("UNRATE") ]
  ...
  if( variable=="3mtbill" ){
    y = data[ -c(1:2), "TB3MS" ]
    X = data[,! colnames(data) %in% c("TB3MS")]
    trans.new = trans[ ! colnames(data) %in% c("TB3MS") ]
  ...
  if( variable=="5yrtbond" ){
    y = data[ -c(1:2), "GS5" ]
    X = data[,! colnames(data) %in% c("GS5")]
    trans.new = trans[ ! colnames(data) %in% c("GS5") ]
  ...
  if (variable == "caputl") {
    y = data[-c(1:2), "CUMFNS"]
    X = data[,!(colnames(data) %in% c("CUMFNS"))]
    trans.new = trans[!(colnames(data) %in% "CUMFNS")]
  ...
  return( list( "trans"=trans.new, "data"=newdata ) )
}
...
rolling.pred = function (macrodata, nprev, i, yar = 6, xar = 6, 
  ...
  cat("iteration:", nprev - i + 1, "c", model_FSR$c_min, 
      ...
      "ALasso", model_alasso$selected, "AIC", model_ar$order, "\n")
...
macro <- read.csv("./High Dim Unit Root/current.csv", header = T)[,-1]

macrodata = data.transform(macro, variable = "unemployment")
macrodata = as.matrix(macrodata$data)

cl = makeCluster(no_clusters)
registerDoParallel(cl)

output = foreach( ii=nprev:1, .combine=rbind,
                  .packages = c( "glmnet", "Ohit", "RcppEigen" ) ) %dopar% {
                    sink()
                    rolling.pred(macrodata = macrodata, nprev = nprev, i = ii,
                                 yar = yar, xar = xar,
...
# 3. Error analysis
pdf("lines_unemp1.pdf", width = 12, height = 8)
...
pdf("barplot_unemp1.pdf", width = 12, height = 6)
...
# 3. Error analysis
pdf("lines_unemp2.pdf", width = 12, height = 8)
plot(acc_sq_err[,1], type = "l", ylim = c(0, max(acc_sq_err)), lwd = 1.5, 
     xlab = "rolling windows", ylab = "accumulated squared error", bty = "n")
pal = rainbow(6)
for (i in 2:7) {
...
pdf("barplot_unemp2.pdf", width = 12, height = 6)
barplot(temp2 / nprev, beside = TRUE, ylab = "frequency",
        names.arg = c("FHTD", "OGA-3", "AR-OGA-3", "LASSO", "ALasso",
                      "AR-ALasso", "AR-AIC"))
dev.off()
